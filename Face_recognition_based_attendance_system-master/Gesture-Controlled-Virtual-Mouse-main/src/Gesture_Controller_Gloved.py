...
class Marker:
    ...
    def detect(self, frame):
        ...
        self.corners, ids, _ = aruco.detectMarkers(gray_frame, self.aruco_dict, parameters=self.parameters)
        if np.all(ids != None):
            _, rvec, tvec = aruco.estimatePoseSingleMarkers(self.corners, 0.05, self.mtx, self.dist)
        else:
            self.corners = None
...
# Rest of the classes and methods remain unchanged...
# I have only corrected the initialization of aruco_dict in the Marker class.
